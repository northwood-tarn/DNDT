...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    let filePath=path.join(toolRoot,safe);
    const stat=await fs.stat(filePath).catch(()=>null);
    if(stat?.isDirectory())filePath=path.join(filePath,"index.html");
    if(!filePath.startsWith(toolRoot))return send(res,403,"Forbidden","text/plain; charset=utf-8");
...
server.listen(port,"127.0.0.1",()=>console.log(`[dialogue-authoring] http://127.0.0.1:${port}/`));
