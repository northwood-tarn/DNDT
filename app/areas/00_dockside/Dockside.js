// app/areas/00_dockside/Dockside.js
...
import { getArea } from "../registry.js";
import { startCombat } from "../../systems/combatLoader.js";
...
function centerBox(text, options=[]) {
  ...
  hint.textContent = "Press a number to choose.";
...
function showEndOfPier() {
  ...
  const options = [
    ...
    { label: "Go to the Second Field", onSelect: () => departToFields() },
    { label: "You hear a noise in a nearby house…", onSelect: () => startCombat('combat_goblins_2') }
  ];
...
function departToFields(){
  const area = getArea('fields');
  if (!area) { console.error("Area 'fields' not found in registry"); return; }
  sceneManager.replace(ExplorationSpriteSceneV2, { tmj: area.tmj, title: 'Fields' });
}
...
try { window.Dockside = Dockside; } catch {}
